...
int WINAPI WinMain(HINSTANCE i, HINSTANCE ii, char *cmd, int sh) {
  ...
  if((f=fopen("Snipes.settings", "rb"))!=0) {
    if(fread(&ll, 2, 1, f)) {
      ll&=65535;
      if((ll&255)>='A'&&(ll&255)<='Z') gameskill=&skills[lastskill=(ll&255)-'A'];
      if((ll>>8)>='0'&&(ll>>8)<='9') gamelevel=&levels[lastlevel=(ll>>8)-'0'];
    }
    fclose(f);
  }
  inst=i;
  ...
  for(o=objs[0].next;o;o=no) {
    ...
    switch(objs[o].id) {
      ...
      case   5: //Snipe
                ...
                if((objs[o].b&2)&&(!rnd(shootrate))&&snitrace(x2, y2, dirxy[d+2], dirxy[d], targbounce)) {
                  objs[o].a=2;
                  objs[o].b&=~2;
                  x=(x2+dirxy[d+2]+mzx)%mzx;
                  y=(y2+dirxy[d]+mzy)%mzy;
                ...
                /////////////////////////////////
                if((objs[o].b&2)&&!objs[o].a) {
                  if(rnd(scanprob)<100&&(co=sniscan(x2, y2, objs[o].d, targbounce))>=0) d=co;
                  else if(!rnd(7)) d=rnd()&7;
